...
public class EnterProfile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String preAct = getIntent().getStringExtra("preAct");

        final EditText nicknameEtext = (EditText) findViewById(R.id.nicknameEtext);

        nicknameEtext.setHint("닉네임을 적어주세요.");
        if(preAct.equals("MainScreen")) {
            Query query = mDatabase.child("Profiles").orderByChild("uid").equalTo(DataManager.getInstance().getUid());
            ...
            nicknameEtext.setEnabled(false);
            nicknameEtext.setClickable(false);
            nicknameEtext.setFocusable(false);
            nicknameEtext.setFocusableInTouchMode(false);
        }
            ...
            public void onClick(View v) {
                if(nicknameEtext.getText().toString().isEmpty() || prof.getLocation().equals("") || prof.getGender().equals("")) {
                    ...
                    query.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            boolean existUser = dataSnapshot.exists();
                            if(existUser && preAct.equals("StartLogin")) {
                                Toast.makeText(EnterProfile.this, "이미 존재하는 닉네임입니다.", Toast.LENGTH_SHORT).show();
                            }
                            else {
                                prof.setNickname(nicknameEtext.getText().toString());
                                prof.setUid(DataManager.getInstance().getUid());
                                prof.setDeviceToken(DataManager.getInstance().getDeviceToken());
                                mDatabase.child("Profiles").child(DataManager.getInstance().getUid()).setValue(prof);


                                DataManager.getInstance().setUid(prof.getUid());
                                DataManager.getInstance().setNickname(prof.getNickname());
                                DataManager.getInstance().setLocation(prof.getLocation());
                                DataManager.getInstance().setAge(prof.getAge());
                                DataManager.getInstance().setGender(prof.getGender());
                                DataManager.getInstance().setDeviceToken(prof.getDeviceToken());
